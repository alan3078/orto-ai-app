...
export class AIConstraintTranslatorService {
  ...
  async translate(text: string): Promise<AITranslationResult> {
    ...
    type VerticalConfig = { time_slot: number | 'ALL'; target_state: number; operator: '>=' | '<=' | '=='; value: number }
    type HorizontalConfig = { resource: string; time_slots: number[]; target_state: number; operator: '>=' | '<=' | '=='; value: number }
    type SlidingConfig = { resource: string; work_days: number; rest_days: number; target_state: number }
    ...
    cleaned.forEach((c) => {
      try {
        ...
        throw new Error(`Constraint validation failed for type ${c.type}: ${(err as Error).message}`)
      }
    })
  ...
  private safeJsonParse(raw: string): unknown | null {
    ...
    const startIdx = startIdxObj === -1 ? startIdxArr : (startIdxArr === -1 ? startIdxObj : Math.min(startIdxObj, startIdxArr))
    const endIdxObj = raw.lastIndexOf('}')
    const endIdxArr = raw.lastIndexOf(']')
    const endIdx = endIdxObj === -1 ? endIdxArr : (endIdxArr === -1 ? endIdxObj : Math.max(endIdxObj, endIdxArr))
