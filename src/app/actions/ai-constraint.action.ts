  ...
  try {
    ...
    const normalized = constraints.map((c) => {
      ...
      if (cfg.resource && typeof cfg.resource === 'string') {
        ...
        if (mapped) {
          ...
          console.warn(`✗ Could not map staff name "${originalResource}". Available staff:`, Array.from(nameToEmployee.keys()))
          throw new Error(`Staff member "${originalResource}" not found. Please use exact staff name or employee ID.`)
        }
      }
      return { ...c, config: cfg }
    })
    ...
    for (const c of normalized) {
      ...
      type SolverShape =
        ...
        | { type: 'vertical_sum'; time_slot: number | 'ALL'; target_state: number; operator: '>=' | '<=' | '=='; value: number }
        | { type: 'horizontal_sum'; resource: string; time_slots: number[]; target_state: number; operator: '>=' | '<=' | '=='; value: number }
        | { type: 'sliding_window'; resource: string; work_days: number; rest_days: number; target_state: number }
      let solverShape: SolverShape
      switch (c.type) {
        case 'point':
          {
            const cfg = c.config as { resource: string; time_slot: number; state: number }
          solverShape = {
            type: 'point',
        ...
        case 'vertical_sum':
          {
            const cfg = c.config as { time_slot: number | 'ALL'; target_state: number; operator: '>=' | '<=' | '=='; value: number }
          solverShape = {
            type: 'vertical_sum',
            time_slot: cfg.time_slot,
        ...
        case 'horizontal_sum':
          {
            const cfg = c.config as { resource: string; time_slots: number[]; target_state: number; operator: '>=' | '<=' | '=='; value: number }
          solverShape = {
            type: 'horizontal_sum',
            resource: cfg.resource,
        ...
        case 'sliding_window':
          {
            const cfg = c.config as { resource: string; work_days: number; rest_days: number; target_state: number }
          solverShape = {
            type: 'sliding_window',
            resource: cfg.resource,
