    ...
    <div className='space-y-6'>
      <div>
        ...
        <p className='text-muted-foreground'>
          Select a staff group, manage active constraints, then view the roster.
        </p>
      </div>
      ...
      {/* Roster Grid */}
      {multiMonthView ? (
        <MultiMonthRosterGrid month={selectedMonth} />
      ) : (
        <RosterGrid month={selectedMonth} />
      )}

      {roster?.id && roster.status === 'COMPLETED' && <ValidationResults rosterId={roster.id} />}
    </div>
  );
}
