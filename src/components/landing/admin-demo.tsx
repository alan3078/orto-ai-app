...
const MOCK_STAFF = [
  { name: 'Sarah Chen', role: 'Senior Nurse', shifts: ['D', 'D', 'D', 'O', 'O', 'N', 'N'] },
  { name: 'James Wilson', role: 'Nurse', shifts: ['N', 'N', 'O', 'O', 'D', 'D', 'D'] },
  { name: 'Maria Garcia', role: 'Junior Nurse', shifts: ['O', 'O', 'A', 'A', 'A', 'O', 'O'] },
  { name: 'David Kim', role: 'Nurse', shifts: ['P', 'P', 'P', 'P', 'O', 'O', 'O'] },
]
...
export function AdminDemo() {
  ...
  const getShiftBadge = (shift: string) => {
    ...
    return (
      <div className={`w-8 h-8 rounded-md flex items-center justify-center font-medium text-sm ${colors[shift] || colors['O']}`}>
        {shift}
      </div>
    )
  }
      ...
      <div className="container px-4 md:px-6 mx-auto">
        <div className="flex flex-col items-center justify-center space-y-4 text-center mb-12">
          ...
          <p className="max-w-[900px] text-muted-foreground md:text-xl/relaxed lg:text-base/relaxed xl:text-xl/relaxed">
            Manage staff, configure shift patterns, and generate AI-optimized schedules from a single intuitive dashboard.
          </p>
        </div>
                    ...
                    <TableBody>
                      {MOCK_CONFIGS.map((config) => (
                        <TableRow key={config.id}>
                          <TableCell className="pl-6">{getShiftTypeBadge(config.shiftType)}</TableCell>
                          <TableCell>
                            <div>
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                      <div className="space-y-2">
                        ...
                        <div className="text-sm text-muted-foreground">
                          Configure rules that apply to all staff members regardless of their shift type.
                        </div>
                      </div>
                      <div className="space-y-2">
                        <Label>Auto-Scheduling</Label>
                        <div className="text-sm text-muted-foreground">
                          Set preferences for the AI solver engine to prioritize fairness or efficiency.
                        </div>
                      </div>
                    </div>
                  </div>
