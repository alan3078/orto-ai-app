...
export function Hero() {
  ...
  return (
    <section ref={ref} className="relative min-h-[110vh] flex items-center justify-center overflow-hidden bg-slate-50">
        ...
        <FloatingCard x={5} y={60} rotate={3} delay={0.6} className="hidden lg:block">
           <div className="bg-white p-3 rounded-lg shadow-md border border-slate-100 w-40 opacity-40 blur-[2px]">
             <div className="grid grid-cols-4 gap-1">
               {[...Array(12)].map((_, i) => (
                 <div key={i} className={`h-2 rounded-sm ${i % 3 === 0 ? 'bg-green-200' : 'bg-slate-100'}`} />
               ))}
             </div>
           </div>
        </FloatingCard>
      ...
      <motion.div style={{ y: yForeground }} className="container mx-auto px-4 relative z-20">
        <div className="max-w-5xl mx-auto text-center">
          ...
          >
            Orto AI transforms complex healthcare staffing requirements into 
            perfectly balanced rosters. Navigate constraints with ease.
          </motion.p>
          ...
          >
            <Button size="lg" className="h-14 px-8 text-lg rounded-full bg-slate-900 hover:bg-slate-800 shadow-xl hover:shadow-2xl transition-all hover:-translate-y-1" asChild>
              <Link href={ROUTES.AUTH.LOGIN}>
                Start Scheduling
                <ArrowRight className="ml-2 h-5 w-5" />
              </Link>
            </Button>
            <Button size="lg" variant="outline" className="h-14 px-8 text-lg rounded-full border-slate-300 hover:bg-slate-50" asChild>
              <Link href={ROUTES.SECTIONS.FEATURES}>
                View Demo
              </Link>
            </Button>
          </motion.div>
      ...
      <motion.div style={{ y: useTransform(scrollYProgress, [0, 1], ["0%", "-20%"]) }} className="absolute inset-0 z-30 pointer-events-none overflow-hidden">
