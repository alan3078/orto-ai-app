...
interface LookupModalProps {
  ...
  isLoading?: boolean
  columns?: { key: keyof Item | 'select'; label: string; widthClass?: string }[]
}
...
export function LookupModal({
  ...
  ].filter(Boolean) as { key: keyof Item | 'select'; label: string; widthClass?: string }[]
    ...
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose() }}>
      <DialogContent className="w-full max-w-xl sm:max-w-2xl lg:max-w-4xl">
                        ...
                        {cols.map(c => (
                          <TableCell key={c.key}>
                            {c.key === 'select' && multiple ? (
                              ...
                              c.key === 'name' ? item.name : c.key === 'email' ? (item.email || '') : c.key === 'description' ? (item.description || '') : ''
                            )}
                          </TableCell>
                        ))}
                      </TableRow>
        ...
        <DialogFooter>
          ...
          {multiple && <Button onClick={handleConfirm} disabled={Object.values(selected).every(v => !v)}>Add Selected</Button>}
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
