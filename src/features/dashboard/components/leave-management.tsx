    ...
    return (leaves as LeaveRecord[]).filter((leave: LeaveRecord) => {
      ...
      if (searchQuery) {
        ...
        return (
          ...
          leave.notes?.toLowerCase().includes(q)
        )
      }
      return true
    })
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filteredLeaves.map((leave: LeaveRecord) => {
                        ...
                        const statusConfig = LEAVE_STATUS_CONFIG[leave.status as keyof typeof LEAVE_STATUS_CONFIG]
                        const staffName = leave.staff?.user?.name || leave.staff?.visibleId || 'Unknown'
