...
export function ShiftDefinitionManagement() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: { code: string; startMinutes: number; durationMinutes: number }) => {
        ...
        <CardHeader>
          <CardTitle className='flex justify-between items-center'>
            ...
            <div className='flex gap-2'>
              <Button size='sm' variant='outline' onClick={() => setAuditDialogOpen(true)}>
              ...
              <TableBody>
                {shifts.map((shift) => (
                  <TableRow key={shift.id}>
                    <TableCell className='font-mono font-bold'>{shift.code}</TableCell>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will permanently delete shift <strong>{deletingShift?.code}</strong>. An audit
              record will be created. This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={auditDialogOpen} onOpenChange={setAuditDialogOpen}>
        <DialogContent className='max-w-2xl'>
          <DialogHeader>
            ...
            <DialogDescription>History of deleted shift definitions</DialogDescription>
          </DialogHeader>
                ...
                <TableBody>
                  {audits.map((audit) => (
                    <TableRow key={audit.id}>
                      <TableCell className='font-mono font-bold'>{audit.code}</TableCell>
                      <TableCell className='text-xs'>{formatTime(audit.startMinutes)}</TableCell>
