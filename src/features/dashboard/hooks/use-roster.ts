...
import {
  ...
  getRosterAction,
  deleteRosterAction,
} from '@/app/actions/generate-roster.action';
...
/**
 * Hook to delete a roster
 */
export function useDeleteRoster() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ rosterId, month }: { rosterId: string; month: string }) => {
      const result = await deleteRosterAction(rosterId);
      if (!result.success) throw new Error(result.error);
      return { month };
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: rosterKeys.month(data.month) });
      toast.success('Roster deleted successfully!');
    },
    onError: (err: Error) => {
      toast.error('Failed to delete roster: ' + err.message);
    },
  });
}
