...
export function useToggleStaffActive() {
  ...
  return useMutation({
    ...
    onMutate: async (id) => {
      ...
      if (previousAll)
        queryClient.setQueryData([...staffKeys.all, 'all'], updateStaffList(previousAll));
      if (previousActive)
        queryClient.setQueryData([...staffKeys.all, 'active'], updateStaffList(previousActive));
      if (previousInactive)
        queryClient.setQueryData([...staffKeys.all, 'inactive'], updateStaffList(previousInactive));
      if (previousDefault)
        queryClient.setQueryData(staffKeys.all, updateStaffList(previousDefault));
    ...
    onError: (err, _id, context) => {
      ...
      if (context?.previousAll)
        queryClient.setQueryData([...staffKeys.all, 'all'], context.previousAll);
      if (context?.previousActive)
        queryClient.setQueryData([...staffKeys.all, 'active'], context.previousActive);
      if (context?.previousInactive)
        queryClient.setQueryData([...staffKeys.all, 'inactive'], context.previousInactive);
      if (context?.previousDefault)
        queryClient.setQueryData(staffKeys.all, context.previousDefault);
